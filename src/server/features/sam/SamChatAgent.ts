...
export class SamChatAgent extends Think {
  ...
  // OpenRouter key + model resolved for THIS request (fetch() prefetches the
  // Settings store, so a key saved in Settings applies on the next turn
  // without touching env). getModel() is sync (Think's hook), hence the
  // instance field instead of an async lookup.
  private aiSettings: {
  ...
  private async refreshAiSettings() {
    try {
      const [apiKey, modelId] = await Promise.all([
        getDynamicSecretValue("OPENROUTER_API_KEY"),
        getDynamicSecretValue("OPENROUTER_MODEL"),
      ]);
      this.aiSettings = {
        apiKey,
      ...
      // Fall back to env-only resolution below.
  ...
  getModel() {
    const apiKey =
      ...
      getEnvValueSync(this.env, "OPENROUTER_API_KEY");
    if (!apiKey) {
      throw new Error("OPENROUTER_API_KEY is required for the SAM agent");
    }
    return buildChatAgentModel(
      apiKey,
      this.aiSettings?.modelId ?? getEnvValueSync(this.env, "OPENROUTER_MODEL"),
    );
  }
  ...
  private buildSoulPrompt(): Promise<string> {
    return withPgClient(async () => {
      ...
      if (!ctx) {
        return "You are SAM, the SEO agent inside AbbaSeo. This chat session no longer exists; tell the user to start a new chat.";
      }
  ...
  async beforeTurn(_ctx: TurnContext): Promise<TurnConfig> {
    ...
    return withPgClient(async (): Promise<TurnConfig> => {
      ...
      const generationSettings = await getDynamicAiGenerationSettings();
