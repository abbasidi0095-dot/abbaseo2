...
export function getDataforseoTaskCount(payload: unknown): number {
  if (!isRecord(payload)) return 1;
  const tasksCount = payload.tasks_count;
  if (
  ...
  return Math.max(1, Math.trunc(tasksCount));
}
...
export async function recordDataforseoQueryCount(
  response: Response,
): Promise<void> {
  try {
    const payload: unknown = await response.json();
    const { incrementAppSettingsQueryCount } =
      await import("@/server/features/settings/SettingsRepository");
    await incrementAppSettingsQueryCount(getDataforseoTaskCount(payload));
  } catch {
    // Usage telemetry must never turn a successful provider request into an
    // application failure.
  }
}
      ...
      if (response.ok) {
        ...
        if (path !== "/v3/appendix/user_data") {
          try {
            ...
            // A telemetry clone failure must not mask the successful response.
          }
        }
        return response;
      }
