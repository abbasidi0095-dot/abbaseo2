...
export async function getChatAgentModel(): Promise<LanguageModelV3> {
  // Settings screen values (OpenRouter key + default model) win over env, so
  // a key saved in Settings applies immediately without touching env files.
  const apiKey = await getDynamicRequiredEnvValue("OPENROUTER_API_KEY");
  const modelId = await getDynamicRequiredEnvValue("OPENROUTER_MODEL").catch(
    () => undefined,
  );
