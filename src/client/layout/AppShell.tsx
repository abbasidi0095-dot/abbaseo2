...
import { StatusBar } from "@/client/layout/StatusBar";
import { BILLING_ROUTE } from "@/shared/billing";
import { getSeoApiKeyStatus } from "@/serverFunctions/config";
import { getProjects } from "@/serverFunctions/projects";
import { getAppBranding } from "@/serverFunctions/settings";
...
export function AuthenticatedAppLayout({
  ...
  const appBrandingQuery = useQuery({
  ...
  const appTitle = appBrandingQuery.data?.appTitle ?? "AbbaSeo";

  // Global ⌘K / Ctrl-K opens the command palette from anywhere in the app.
  // Escape closes it (and never fights the switcher/modal Escape handlers —
  // those stop propagation at their own roots).
  React.useEffect(() => {
  ...
  return (
    <div className="flex h-[100dvh] bg-base-200">
      <div className="hidden shrink-0 md:block">
        <Sidebar projectId={sidebarProjectId} appTitle={appTitle} />
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <StatusBar onOpenCommandPalette={() => setPaletteOpen(true)} />
