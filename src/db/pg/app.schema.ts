...
// Deployment-wide application settings (DataForSEO credentials, AI provider
// keys, branding/preferences) managed from the Settings screen. A single row
// (`id = "app"`) holds the whole payload as one JSON string; secret fields are
// encrypted with BETTER_AUTH_SECRET when it is configured (same symmetric
// helper as the GSC OAuth tokens). One row per deployment — mirrors the old
// single-env-var model, so reads consult the store first and fall back to env.
