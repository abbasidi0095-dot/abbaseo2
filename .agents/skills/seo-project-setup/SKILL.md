...
### 5. Verify AbbaSeo MCP

After the user has described the company, website, goals, and positioning, check that AbbaSeo MCP is configured and mapped to the right project:
...
5. If the MCP is unavailable, tell the user to connect AbbaSeo MCP before continuing with live AbbaSeo data.
...
After intake, recommend one next AbbaSeo workflow:
