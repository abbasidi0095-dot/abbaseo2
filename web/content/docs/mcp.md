---
title: "Set up AbbaSeo MCP"
description: "Connect AbbaSeo MCP to Claude, Codex, and other AI clients."
---

AbbaSeo MCP lets compatible AI clients call AbbaSeo tools for keyword research, SERP inspection, local business research, competitive search intelligence, domain research, backlink overview, saved keywords, rank tracking, and Google Search Console performance and URL inspection.
...
The first connection sends you through AbbaSeo login. After authorization, your MCP client can call AbbaSeo tools with the project context and account scopes you approved.

For the most current setup UI and a copyable endpoint, open [AI & MCP in AbbaSeo](https://app.openseo.so/ai).

## Claude Code

Use user scope to make AbbaSeo available across projects. Use local scope for the current repository.

```bash
claude mcp add --transport http --scope user openseo https://app.openseo.so/mcp
```

After adding the server, approve the AbbaSeo login when prompted.
...
4. Approve the AbbaSeo login when prompted.
...
AbbaSeo MCP exposes tools for SEO research workflows:
...
- List saved keywords from an AbbaSeo project.
- Save useful keywords back to AbbaSeo.
...
Once AbbaSeo MCP is connected, [set up AbbaSeo Agent Skills](/docs/skills/setup). MCP gives your agent access to AbbaSeo data. Skills are separate `SKILL.md` files that tell your agent how to use that data for specific SEO jobs.
...
If authorization fails, disconnect the AbbaSeo server in your client, add it again, and repeat the login flow.

If your agent cannot find a project, ask it to list AbbaSeo projects first and use the returned project ID in later tool calls.
