...
description: "Deploy AbbaSeo to your own Cloudflare account for internet-facing, multi-device, or team use."
---

Host AbbaSeo on Cloudflare for internet-facing self-hosting across multiple devices or with your team. One deploy command provisions everything, including the Cloudflare Access login gate. Works on Cloudflare's free plan.
...
3. AbbaSeo should load after login.

If login fails, re-check `ACCESS_ALLOWED_EMAILS` and redeploy.

## Connect the MCP server through Cloudflare Access

Use the same Cloudflare Access application that protects your AbbaSeo Worker. Managed OAuth is required for MCP clients and is not enabled by default.

1. Open Cloudflare Zero Trust.
2. Go to `Access controls` -> `Applications`.
3. Find your AbbaSeo application, then select `Edit`.
...
## Give teammates access to AbbaSeo

Add the teammate to `ACCESS_ALLOWED_EMAILS` in `.env.selfhost` and redeploy. Everyone allowed through shares one AbbaSeo workspace.

## Updating to the latest AbbaSeo version
