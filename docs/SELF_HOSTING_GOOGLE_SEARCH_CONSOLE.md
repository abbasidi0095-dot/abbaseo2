...
Connecting Google Search Console (GSC) lets AbbaSeo pull your real clicks,
impressions, positions, and URL inspection data, straight from Google.

It's **optional**: AbbaSeo runs fine without it, just without Search Console data.
...
Set these three values, then restart AbbaSeo:
...
Restart AbbaSeo so it picks up the new variables. For Docker, changing `.env`
...
- AbbaSeo uses your Google client to run the OAuth flow and stores the resulting
...
- Search Console data comes from your own Google account, so AbbaSeo never meters credits for it.
